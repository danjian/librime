...
static void rime_fill_preedit_syllables(Context* ctx,
  ...
  size_t input_pos = 0;
  const size_t input_length = comp.input().length();
  auto append_unsegmented = [&syllables, &comp, input_length](size_t start,
                                                              size_t end) {
    start = std::min(start, input_length);
    end = std::min(end, input_length);
    if (start < end)
      syllables.emplace_back(comp.input().substr(start, end - start), "", "");
  };
  for (const auto& seg : comp) {
    append_unsegmented(input_pos, seg.start);
    auto cand = seg.GetSelectedCandidate();
    if (!cand) {
      append_unsegmented(seg.start, seg.end);
      input_pos = std::max(input_pos, seg.end);
      continue;
    }
    ...
    size_t seg_end_idx = syllables.size();
    append_unsegmented(pos, seg.end);
    if (confirmed && seg_end_idx > seg_start_idx)
      confirmed_ranges.emplace_back(seg_start_idx, seg_end_idx - 1);
    input_pos = std::max(input_pos, seg.end);
  }
  append_unsegmented(input_pos, input_length);
  if (syllables.empty())
    return;
...
RIME_API RIME_FLAVORED(RimeApi) * RIME_FLAVORED(rime_get_api)() {
  ...
  if (!s_api.data_size) {
    ...
    s_api.highlight_candidate_on_current_page =
     ...
     s_api.append_input = &RimeAppendInput;
  }
  return &s_api;
}
